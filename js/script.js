...
document.addEventListener("DOMContentLoaded", () => {
    // Busca o parágrafo que fica logo após o seu título H1
    const introElement = document.querySelector('.h1Titulo + p');
    
    if (introElement) {
        // Salva o texto original e limpa a tela
        ...
        function typeWriter() {
            if (i < text.length) {
                ...
                i++;
                setTimeout(typeWriter, 25); // Velocidade da digitação em milissegundos
            }
        }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (phoneInput) {
        phoneInput.addEventListener('input', function (e) {
            ...
            // Monta a formatação dinamicamente
...
window.addEventListener('scroll', () => {
    ...
    if (window.scrollY > 300) {
        if (!btn) {
            ...
            // Estilização do botão via JS para facilitar sua vida
            btn.style.cssText = `
                position: fixed;
                bottom: 20px;
                right: 20px;
        ...
        // Esconde o botão se voltar pro topo
...
// ==========================================
document.addEventListener("DOMContentLoaded", () => {
    // Verifica se estamos na página index (procurando os cards do Github)
    const githubStatsImg = document.querySelector('.cardsGithub');
    
    if (githubStatsImg) {
        // Cria uma nova div para colocar os repositórios reais logo abaixo das suas estatísticas
        ...
        fetch('https://api.github.com/users/silasgoncalvesczs/repos?sort=updated&per_page=3')
            ...
            .then(data => {
                data.forEach(repo => {
                    ...
                    // Estrutura do card com nome, descrição e botão de link
                    card.innerHTML = `
